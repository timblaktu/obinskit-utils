...
originalMap from dighelm project was "syntactically pythonified".

We transform `originalMap` into separate useful maps keyed by
name and value, repectively:
    1. keycodes_by_value
    2. keycodes_by_name
...
Nota Bene:
    * "value" in this context is the integer that appears in macro_value
      in the SQLITE db.
    * there were DUPLICATES IN THE ORIGINALMAP for:
        * ENTER and 0-9:
              These all appear in the [88-98] range. I modified the code
              below to simply not insert if already in the map.
        * CTRL, ALT, SHIFT:
              These appear to be not duplicates but Left/Right variants.
              For these, I corrected their names in originalMap to
              disambiguate. This was previously stored in the 'info' field.
    * Ignored unused/unknown elements in originalMap (alias, id, info, icon...)
...
originalMap = [
    ...
    [42, {"name": "LeftShift", "info": "LeftShift", "value": 225}],
    [43, {"name": "\\|", "value": 49}],
    [
        44,
        {
    ...
    [54, {"name": "RightShift", "info": "RightShift", "value": 229}],
    [
        56,
        {
            "name": "LeftAlt",
    ...
    #     },
    # ],
    [3613, {"name": "RightCtrl", "info": "RightCtrl", "value": 228}],
    [
        3639,
        {
            "name": "PrintScreen",
    ...
    [3640, {"name": "RightAlt", "info": "RightAlt", "value": 230}],
    [
        3653,
        {
            "name": "Pause",
...
keycodes_by_value = { _dict["value"]:{'id': _id, 'name':_dict['name'] } for _id, _dict in originalMap }
keycodes_by_name = { _dict["name"]:{'id': _id, 'value':_dict['value'] } for _id, _dict in originalMap }
